...
use crate::config::{components::OciAuthentication, plugins::ArkPlugin};
...
impl UriHandler for OciHandler {
    ...
    async fn get(&self, plugin_config: &ArkPlugin) -> anyhow::Result<PluginLoadResult> {
        ...
        let wasm = WasmHandler::new(wasm_bytes, &plugin_config.manifest)?; // Validates module and required exports

        // Execute plugin
        let exec_start = Instant::now();
        let result = wasm.get(plugin_config).await?;
        debug!(
            repo = LOCAL_LOG_PREFIX,
